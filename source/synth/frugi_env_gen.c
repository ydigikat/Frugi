 ...
 */
static void recalc_coefficients(struct frugi_env_gen *env_gen)
{
  float attack_blocks = env_gen->attack * env_gen->attack_scaler * env_gen->fsr / (1000.0f * env_gen->block_size);
  float decay_blocks = env_gen->decay * env_gen->decay_scaler * env_gen->fsr / (1000.0f * env_gen->block_size);
  float release_blocks = env_gen->release * env_gen->fsr / (1000.0f * env_gen->block_size);
  ...
  env_gen->decay_overshoot = (env_gen->sustain - env_gen->decay_tco) * (1.0f - env_gen->decay_coeff);
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
static void env_state_off(struct frugi_env_gen *env_gen)
{
  env_gen->level = 0.0f;
}
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
static void env_state_attack(struct frugi_env_gen *env_gen)
{
  env_gen->level = env_gen->level * env_gen->attack_coeff + env_gen->attack_overshoot;

  

  if (env_gen->level >= 1.0f || env_gen->attack <= 0.0f)
  {
    env_gen->level = 1.0f;
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
static void env_state_decay(struct frugi_env_gen *env_gen)
{
  env_gen->level = env_gen->level * env_gen->decay_coeff + env_gen->decay_overshoot;

  

  if (env_gen->level <= env_gen->sustain || env_gen->decay <= 0.0f)
  {
    env_gen->level = env_gen->sustain;
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
static void env_state_sustain(struct frugi_env_gen *env_gen)
{
  env_gen->level = env_gen->sustain;
}
 ...
 * \param env_gen Pointer to the envelope generator instance 
 */
static void env_state_release(struct frugi_env_gen *env_gen)
{
  env_gen->level = env_gen->level * env_gen->release_coeff + env_gen->release_overshoot;

  if (env_gen->level <= 0.0f || env_gen->release <= 0.0f)
  {
    env_gen->level = 0.0f;
 ...
 * \param env_gen Pointer to the envelope generator instance 
 */
static void env_state_shutdown(struct frugi_env_gen *env_gen)
{
...
static void (*env_state_handlers[ENV_MAX])(struct frugi_env_gen *env_gen) =
    {
        env_state_off,
        env_state_attack,
        env_state_decay,
...
/**
 * frugi_env_gen_init
 * \brief Initializes a new envelope generator instance with default settings
 * \param env_gen Pointer to the envelope generator instance to initialize
 * \param fsr Sample rate in Hz (e.g., 44100, 48000)
 * \param block_size Number of samples per processing block
 * \param output Pointer to the output buffer where envelope values will be written
 */
void frugi_env_gen_init(struct frugi_env_gen *env_gen, float fsr, size_t block_size, float *output)
{
  RTT_ASSERT(env_gen != NULL);
  RTT_ASSERT(output != NULL);
  ...
  env_gen->output = output;

  frugi_env_gen_reset(env_gen);
}

/**
 * frugi_env_gen_reset
 * \brief Resets the envelope generator to its initial OFF state
 * \note Sets level to zero and state to OFF without changing parameters
 * \param env_gen Pointer to the envelope generator instance to reset
 */
void frugi_env_gen_reset(struct frugi_env_gen *env_gen)
{
  RTT_ASSERT(env_gen != NULL);
 ...
 */
void frugi_env_gen_render(struct frugi_env_gen *env_gen, size_t block_size)
{
  RTT_ASSERT(env_gen != NULL);
  RTT_ASSERT(env_gen->output != NULL);

  env_state_handlers[env_gen->state](env_gen);
  *env_gen->output = env_transforms[env_gen->mode](env_gen->level, env_gen->sustain);
}

/**
 * frugi_env_gen_note_on
 ...
 */
void frugi_env_gen_note_on(struct frugi_env_gen *env_gen, uint8_t midi_note, uint8_t midi_velocity)
{
  RTT_ASSERT(env_gen != NULL);

  /* Store the scaling (tracking) values */
  env_gen->attack_scaler = env_gen->velocity_tracking ?  1.0f - midi_velocity / 127.0f : 1.0f;
  env_gen->decay_scaler = env_gen->note_tracking ? 1.0f - midi_note / 127.0f : 1.0f;
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
void frugi_env_gen_note_off(struct frugi_env_gen *env_gen)
{
  RTT_ASSERT(env_gen != NULL);
 ...
 */
void frugi_env_gen_update_params(struct frugi_env_gen *env_gen, float attack, float decay, float sustain, float release, float mode, float note_tracking, float velocity_tracking)
{
  RTT_ASSERT(env_gen != NULL);

  env_gen->attack = PARAM_TO_POWER(attack, ENV_ATTACK_MS_MIN, ENV_ATTACK_MS_MAX, ENV_POWER_EXP);
  env_gen->decay = PARAM_TO_POWER(decay, ENV_DECAY_MS_MIN, ENV_DECAY_MS_MAX, ENV_POWER_EXP);
  env_gen->sustain = sustain;
  env_gen->release = PARAM_TO_POWER(release, ENV_RELEASE_MS_MIN, ENV_RELEASE_MS_MAX, ENV_POWER_EXP);
  env_gen->mode = mode;
  env_gen->note_tracking = note_tracking;
  env_gen->velocity_tracking = velocity_tracking;
 ...
 * \param env_gen Pointer to the envelope generator instance
 */
void frugi_env_gen_rtz(struct frugi_env_gen *env_gen)
{
  RTT_ASSERT(env_gen != NULL);
